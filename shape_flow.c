...
static int has_duplicate_points(Point *pts, int n) {
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
...
static int quad_has_any_collinear_triple(Point *pts) {
    // 4점 중 어떤 3점이라도 일직선이면(퇴화) 사각형으로 보지 않음
    Point A = pts[0], B = pts[1], C = pts[2], D = pts[3];
    return are_collinear(A, B, C) || are_collinear(A, B, D) ||
           are_collinear(A, C, D) || are_collinear(B, C, D);
}
...
void handle_quadrilateral(void) {
    ...
    for (;;) {
        ...
        if (has_duplicate_points(pts, 4)) {
            printf("중복된 점이 있어 사각형이 아닙니다. 다시 입력하세요.\n");
            continue;
        }
        if (quad_has_any_collinear_triple(pts)) {
            printf("세 점이 일직선상에 있어 사각형이 아닙니다. 다시 입력하세요.\n");
            continue;
        }
